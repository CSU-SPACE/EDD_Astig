...
class AssociativeEmbeddingHead(HeatmapHead):
    ...
    def _flip_tags(
        self, tags: Tensor, flip_indices: List[int], shift_heatmap: bool = True
    ):
    ...
    def decode(self, batch_outputs: Union[Tensor, Tuple[Tensor]]) -> InstanceList:
        ...
        preds = [
            InstanceData(
                bbox_scores=instance_scores, keypoints=keypoints, keypoint_scores=scores
            )
    ...
    def loss(
        ...
        loss_kpt = self.loss_module.keypoint_loss(
            pred_heatmaps, gt_heatmaps, keypoint_weights, gt_masks
        )

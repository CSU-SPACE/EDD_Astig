...
class IIAModule(BaseModule):
    ...
    def _sample_feats(self, feats: Tensor, indices: Tensor) -> Tensor:
        ...
        if indices.shape[1] == 3:
            ...
            raise ValueError(
                f"`indices` should have 2 or 3 channels, "
    ...
    def forward_train(
        self, feats: Tensor, instance_coords: Tensor, instance_imgids: Tensor
    ) -> Tuple[Tensor, Tensor]:
...
class SpatialAttention(nn.Module):
    ...
    def forward(
        self, global_feats: Tensor, instance_feats: Tensor, instance_coords: Tensor
    ) -> Tensor:
        ...
        relative_coords = instance_coords.reshape(-1, 1, 2) - pixel_coords.reshape(
            1, -1, 2
        )
    ...
    def forward(
        ...
        cond_instance_feats = torch.cat(
            (
                ...
                self.spatial_attention(global_feats, instance_feats, instance_coords),
            ),
            dim=1,
        )
...
class CIDHead(BaseHead):
        ...
        self.gfd_module = GFDModule(
            ...
            + [
                dict(
                    ...
                    std=0.001,
                    override=dict(
                        name="heatmap_conv", type="Normal", std=0.001, bias=bias_value
                    ),
                )
            ],
        )
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tensor:
        ...
        instance_heatmaps = self.gfd_module(
            feats, instance_feats, instance_coords, instance_imgids
        )
    ...
    def predict(
        ...
        if len(instance_coords) > 0:
            ...
            preds.keypoints[..., 0] += (
                metainfo["input_size"][0] / instance_heatmaps.shape[-1] / 2.0
            )
            preds.keypoints[..., 1] += (
                metainfo["input_size"][1] / instance_heatmaps.shape[-2] / 2.0
            )
            preds = [preds]
    ...
    def loss(
        ...
        for i, d in enumerate(batch_data_samples):
            ...
            instance_imgids.append(
                torch.ones(len(d.gt_instance_labels.instance_coords), dtype=torch.long)
                * i
            )
        ...
        if max_train_instances > 0 and len(instance_imgids) > max_train_instances:
            selected_indices = torch.randperm(
                len(instance_imgids), device=gt_heatmaps.device, dtype=torch.long
            )[:max_train_instances]
        ...
        pred_instance_heatmaps = self.gfd_module(
            feats, pred_instance_feats, gt_instance_coords, instance_imgids
        )

        # calculate losses
        losses = {
            "loss/heatmap_coupled": self.loss_module["heatmap_coupled"](
                pred_heatmaps, gt_heatmaps, None, heatmap_mask
            )
        }

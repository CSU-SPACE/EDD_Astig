...
class RescoreNet(BaseModule):
    ...
    def make_feature(self, keypoints, keypoint_scores, skeleton):
        ...
        normalize = (
            joint_length[:, self.norm_indexes[0]]
            + joint_length[:, self.norm_indexes[1]]
        ) / 2
        ...
        feature = torch.cat(
            (joint_relate, joint_length, keypoint_scores), dim=1
        ).float()
        return feature
...
class DEKRHead(BaseHead):
    ...
    def __init__(
        ...
        displacement_loss: ConfigType = dict(
            type="SoftWeightSmoothL1Loss", use_target_weight=True, supervise_empty=False
        ),
    ...
    def _make_heatmap_conv_layers(
        self, in_channels: int, out_channels: int, num_filters: int
    ):
    ...
    def _make_displacement_conv_layers(
        self, in_channels: int, out_channels: int, num_filters: int, groups: int
    ):
    ...
    def predict(
        ...
        if test_cfg.get("output_heatmaps", False):
            ...
            for i in range(B):
                pred_fields.append(
                    PixelData(
                        heatmaps=heatmaps[0][i], displacements=displacements[0][i]
                    )
                )

...
class DCC(BaseModule):
    ...
    def _get_bin_enc(self, bbox_cs, grids):
        ...
        x_bins = (
            x_bins.view(*((1,) * (scale.ndim - 1)), -1) * scale[..., 0:1]
            + center[..., 0:1]
        )
        y_bins = (
            y_bins.view(*((1,) * (scale.ndim - 1)), -1) * scale[..., 1:2]
            + center[..., 1:2]
        )
    ...
    def _pose_feats_to_heatmaps(self, pose_feats, x_bins_enc, y_bins_enc):
        ...
        kpt_feats = kpt_feats.reshape(
            *kpt_feats.shape[:-1], self.num_keypoints, self.feat_channels
        )
    ...
    def generate_target_heatmap(self, kpt_targets, bbox_cs, sigmas, areas):
        ...
        center, scale = bbox_cs.split(2, dim=-1)
        x_bins = (
            self.x_bins.view(*((1,) * (scale.ndim - 1)), -1) * scale[..., 0:1]
            + center[..., 0:1]
        )
        y_bins = (
            self.y_bins.view(*((1,) * (scale.ndim - 1)), -1) * scale[..., 1:2]
            + center[..., 1:2]
        )
...
class RTMOHead(YOLOXPoseHead):
    ...
    def loss(
        ...
        flatten_bbox_decoded = self.decode_bbox(
            flatten_bbox_preds, flatten_priors[..., :2], flatten_priors[..., -1]
        )
        flatten_kpt_decoded = self.decode_kpt_reg(
            flatten_kpt_offsets, flatten_priors[..., :2], flatten_priors[..., -1]
        )
        ...
        num_pos = torch.tensor(
            sum(num_fg_imgs), dtype=torch.float, device=flatten_cls_scores.device
        )
        ...
        if num_pos > 0:
            ...
            losses["loss_bbox"] = (
                self.loss_bbox(bbox_preds, bbox_targets) / num_total_samples
            )
            ...
            if self.use_aux_loss:
                if hasattr(self, "loss_bbox_aux"):
                    ...
                    losses["loss_bbox_aux"] = (
                        self.loss_bbox_aux(bbox_preds_raw, bbox_aux_targets)
                        / num_total_samples
                    )
            ...
            # 3.3 keypoint loss
            kpt_reg_preds = flatten_kpt_decoded.view(-1, self.num_keypoints, 2)[
                pos_masks
            ]

            if hasattr(self, "loss_mle") and self.loss_mle.loss_weight > 0:
                pose_vecs = flatten_pose_vecs.view(-1, flatten_pose_vecs.size(-1))[
                    pos_masks
                ]
                bbox_cs = torch.cat(bbox_xyxy2cs(bbox_preds, self.bbox_padding), dim=1)
                # 'cc' refers to 'cordinate classification'
                kpt_cc_preds, pred_hms, sigmas = self.dcc.forward_train(
                    pose_vecs, bbox_cs, pos_priors[..., :2]
                )
            ...
            if self.proxy_target_cc:
                ...
                with torch.no_grad():
                    ...
                    oks = self.assigner.oks_calculator(
                        kpt_cc_preds, kpt_targets, vis_targets, pos_areas
                    )
                    cls_targets = oks.unsqueeze(1)

                losses["loss_oks"] = self.loss_oks(
                    kpt_reg_preds, kpt_cc_preds.detach(), kpt_weights_reg, pos_areas
                )

            else:
                losses["loss_oks"] = self.loss_oks(
                    kpt_reg_preds, kpt_targets, vis_targets, pos_areas
                )
        ...
        losses["loss_cls"] = (
            self.loss_cls(cls_preds_all, obj_targets, obj_weights) / num_total_samples
        )
    ...
    def predict(
        ...
        flatten_bbox_preds = self.decode_bbox(
            flatten_bbox_preds, flatten_priors, flatten_stride
        )

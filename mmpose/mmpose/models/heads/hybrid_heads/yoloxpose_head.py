...
class YOLOXPoseHead(BaseModule):
    ...
    def loss(
        ...
        losses["loss_obj"] = (
            self.loss_obj(obj_preds, obj_targets, obj_weights) / num_total_samples
        )
        ...
        if num_pos > 0:
            ...
            losses["loss_kpt"] = self.loss_oks(
                kpt_preds, kpt_targets, vis_targets, pos_areas
            )
            ...
            losses["loss_cls"] = (
                self.loss_cls(cls_preds, cls_targets) / num_total_samples
            )
            ...
            if self.use_aux_loss:
                ...
                if hasattr(self, "loss_kpt_aux"):
                    ...
                    kpt_preds_raw = flatten_kpt_offsets.view(-1, self.num_keypoints, 2)[
                        pos_masks
                    ]
    ...
    def _get_targets(
        ...
        if self.use_aux_loss:
            ...
            kpt_aux_targets = (kpt_targets - pos_priors[:, None, :2]) / pos_priors[
                :, None, 2:
            ]
        else:
    ...
    def _get_targets_single(
        ...
        cls_target = F.one_hot(
            pos_gt_labels, self.num_classes
        ) * max_overlaps.unsqueeze(-1)
    ...
    def predict(
        ...
        flatten_kpt_reg = self.decode_kpt_reg(
            flatten_kpt_offsets, flatten_priors, flatten_stride
        )

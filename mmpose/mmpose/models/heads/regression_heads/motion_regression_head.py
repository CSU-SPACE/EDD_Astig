...
class MotionRegressionHead(BaseHead):
    ...
    def __init__(
        ...
        self.pre_logits = nn.Sequential(
            OrderedDict(
                [("fc", nn.Linear(in_channels, embedding_size)), ("act", nn.Tanh())]
            )
        )
        self.fc = (
    ...
    def predict(
        ...
        if camera_param is not None:
            ...
            w = torch.stack(
                [torch.empty((0), dtype=torch.float32) for _ in batch_data_samples]
            )
            h = torch.stack(
                [torch.empty((0), dtype=torch.float32) for _ in batch_data_samples]
            )
        ...
        if factor is not None:
            factor = torch.stack(
                [torch.from_numpy(b.metainfo["factor"]) for b in batch_data_samples]
            )
        else:
            factor = torch.stack(
                [torch.empty((0), dtype=torch.float32) for _ in batch_data_samples]
    ...
    def loss(
        ...
        lifting_target_label = torch.stack(
            [d.gt_instance_labels.lifting_target_label for d in batch_data_samples]
        )
        lifting_target_weight = torch.stack(
            [d.gt_instance_labels.lifting_target_weight for d in batch_data_samples]
        )

        # calculate losses
        losses = dict()
        loss = self.loss_module(
            pred_outputs, lifting_target_label, lifting_target_weight.unsqueeze(-1)
        )

...
class EDPoseDecoder(BaseModule):
    ...
    def __init__(
        ...
        self.ref_point_head = FFN(
            self.query_dim // 2 * self.embed_dims, self.embed_dims, self.embed_dims, 2
        )
        ...
        self.kpt_index = [
            x
            for x in range(self.num_group * (self.num_keypoints + 1))
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if layer_id == self.num_box_decoder_layers - 1:
                ...
                topk_proposals = torch.topk(
                    class_unselected.max(-1)[0], inter_select_number, dim=0
                )[1]
                ...
                new_output_for_keypoint = (
                    new_output_for_box[:, None, :, :]
                ...
                keypoint_xy = (
                    inverse_sigmoid(new_reference_points_for_box[..., :2][:, None])
                    + delta_xy
                ).sigmoid()
                num_queries, _, bs, _ = keypoint_xy.shape
                keypoint_wh_weight = (
                    self.hw.weight.unsqueeze(0)
                    .unsqueeze(-2)
                    .repeat(num_queries, 1, bs, 1)
                    .sigmoid()
                )
                keypoint_wh = (
                    keypoint_wh_weight * new_reference_points_for_box[..., 2:][:, None]
                )
            ...
            if layer_id >= self.num_box_decoder_layers:
                ...
                ref_before_sigmoid_bbox_norm = ref_before_sigmoid[effect_num_dn:][
                    0 :: (self.num_keypoints + 1)
                ]
                ...
                delta_xy_unsig = self.pose_embed[
                    layer_id - self.num_box_decoder_layers
                ](output_kpt)
                outputs_unsig = (
                    ref_before_sigmoid[effect_num_dn:]
                    .index_select(0, torch.tensor(self.kpt_index, device=output.device))
                    .clone()
                )
                ...
                new_reference_points = torch.cat(
                    (new_reference_points_for_box_dn, new_reference_points_norm), dim=0
                )
    ...
    def get_proposal_pos_embed(
        pos_tensor: Tensor, temperature: int = 10000, num_pos_feats: int = 128
    ) -> Tensor:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if num_keypoints == 17:
            ...
            if dec_pred_pose_embed_share:
                pose_embed_layerlist = [
                    _pose_embed for i in range(num_pred_layer - num_box_decoder_layers)
                ]
            else:
                pose_embed_layerlist = [
                    copy.deepcopy(_pose_embed)
        ...
        pose_hw_embed_layerlist = [
            _pose_hw_embed for i in range(num_pred_layer - num_box_decoder_layers)
        ]
    ...
    def forward(
        ...
        for dec_lid, (
            ...
            if dec_lid < self.num_box_decoder_layers:
                ...
                layer_hs_bbox_norm = layer_hs[:, effec_dn_num:, :][
                    :, 0 :: (self.num_keypoints + 1), :
                ]
                bs = layer_ref_sig.shape[0]
                ref_before_sigmoid_bbox_dn = layer_ref_sig[:, :effec_dn_num, :]
                ref_before_sigmoid_bbox_norm = layer_ref_sig[:, effec_dn_num:, :][
                    :, 0 :: (self.num_keypoints + 1), :
                ]
        ...
        for dec_lid, (layer_ref_sig, layer_hs) in enumerate(
            ...
            if dec_lid < self.num_box_decoder_layers:
                ...
                layer_res = xyv.reshape(
                    (bs, self.num_group, self.num_keypoints, 3)
                ).flatten(2, 3)
...
class EDPoseHead(TransformerHead):
    ...
    def pre_transformer(
        ...
        if self.refine_queries_num > 0 or batch_data_samples is not None:
            ...
            input_query_bbox = input_query_label = humandet_attn_mask = (
                human2pose_attn_mask
            ) = mask_dict = None
        ...
        if self.as_two_stage:
            ...
            if input_query_bbox is not None:
                reference_points = torch.cat(
                    [input_query_bbox, topk_coords_unact], dim=1
                ).sigmoid()
            ...
            query = (
                self.query_embedding.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1)
            )
            reference_points = self.refpoint_embedding.weight[:, None, :].repeat(
                1, bs, 1
            )
    ...
    def forward_decoder(
        ...
        """
        decoder_in, head_in = self.pre_decoder(
            memory, memory_mask, spatial_shapes, input_query_bbox, input_query_label
        )
        ...
        decoder_outputs_dict = dict(
            hidden_states=inter_states, references=references, mask_dict=mask_dict
        )
    ...
    def decode(
        ...
        for input_shape, pred_logit, pred_bbox, pred_kpts in zip(
            ...
            preds.append(
                InstanceData(
                    keypoints=keypoints, keypoint_scores=keypoint_scores, bboxes=bboxes
                )
            )
    ...
    def prepare_for_denosing(self, targets: OptSampleList, device):
        ...
        def get_indices_for_repeat(now_num, target_num, device="cuda"):
            ...
            out_indice.append(
                base_indice[torch.randint(0, now_num, (residue,), device=device)]
            )
        ...
        for idx, (gt_boxes_i, gt_labels_i, gt_keypoint_i) in enumerate(
            ...
            if num_gt_i > 0:
                ...
                gt_labels_expand_i = torch.ones(
                    refine_queries_num, dtype=torch.int64, device=device
        ...
        if self.denosing_cfg["dn_box_noise_scale"] > 0:
            ...
            knwon_boxes_expand += (
                torch.mul((torch.rand_like(knwon_boxes_expand) * 2 - 1.0), diff)
                * self.denosing_cfg["dn_box_noise_scale"]
            )
        ...
        if "group2group" in self.denosing_cfg["dn_attn_mask_type_list"]:
            ...
            for matchj in range(self.num_group * (self.num_keypoints + 1)):
                ...
                ej = (matchj // group_bbox_kpt + 1) * group_bbox_kpt
                if sj > 0:
                    attn_mask2[:, :, refine_queries_num:, refine_queries_num:][
                        :, :, matchj, :sj
                    ] = True
                if ej < self.num_group * (self.num_keypoints + 1):
                    attn_mask2[:, :, refine_queries_num:, refine_queries_num:][

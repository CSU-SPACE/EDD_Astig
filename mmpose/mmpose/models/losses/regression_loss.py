...
class SoftWeightSmoothL1Loss(nn.Module):
    ...
    def __init__(
        self, use_target_weight=False, supervise_empty=True, beta=1.0, loss_weight=1.0
    ):
        super().__init__()
    ...
    def smooth_l1_loss(input, target, reduction="none", beta=1.0):
        ...
        if reduction == "mean":
            ...
            raise ValueError(
                f"reduction must be 'mean', 'sum' or " f"'none', but got '{reduction}'"
            )
...
class MPJPEVelocityJointLoss(nn.Module):
    ...
    def forward(self, output, target, target_weight=None):
        ...
        norm_output = torch.mean(
            torch.sum(torch.square(output), dim=-1, keepdim=True), dim=-2, keepdim=True
        )
        norm_target = torch.mean(
            torch.sum(target * output, dim=-1, keepdim=True), dim=-2, keepdim=True
        )
        ...
        if self.use_target_weight:
            ...
            nmpjpe = torch.mean(
                torch.norm(
                    (norm_target / norm_output * output - target) * target_weight,
                    dim=-1,
                )
            )

            loss_3d_velocity = torch.mean(
                torch.norm((velocity_output - velocity_target) * target_weight, dim=-1)
            )
        else:
            mpjpe = torch.mean(torch.norm(output - target, dim=-1))

            nmpjpe = torch.mean(
                torch.norm(norm_target / norm_output * output - target, dim=-1)
            )

    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.multi_scale:
            ...
            features = [int(embed_dim * 2**i) for i in range(1, self.num_layers)] + [
                self.num_features
            ]
            ...
            if upsample == "deconv":
                ...
                self.upsample = nn.Sequential(
                    ...
                    nn.Conv2d(
                        self.num_features, self.num_features, 3, stride=1, padding=1
                    ),
    ...
    def forward_features(self, x):
        ...
        if self.multi_scale:
            ...
            for i, layer in enumerate(self.layers):
                x = layer(x)
                x_2d = x.view(
                    B, H // (8 * self.scales[i]), W // (8 * self.scales[i]), -1
                ).permute(
                    0, 3, 1, 2
                )  # B C H W
            ...
            x = self.norm(x)  # B L C
            x = x.view(B, H // 8, W // 8, self.num_features).permute(
                0, 3, 1, 2
            )  # B C H W
            ...
            for layer in self.layers:
                x = layer(x)
            x = self.norm(x)  # B L C
            x = x.view(B, H // 32, W // 32, self.num_features).permute(
                0, 3, 1, 2
            )  # B C H W
            x = self.upsample(x)

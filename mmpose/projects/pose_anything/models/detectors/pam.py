...
class PoseAnythingModel(BaseModel, metaclass=ABCMeta):
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        target_sizes = (
            torch.tensor([img_q.shape[-2], img_q.shape[-1]])
            .unsqueeze(0)
            .repeat(img_q.shape[0], 1, 1)
        )
    ...
    def forward_test(
        ...
        **kwargs,
        ...
        result.update(
            {
                "points": torch.cat((initial_proposals, output.squeeze(1)), dim=0)
                .cpu()
                .numpy()
            }
        )
    ...
    def extract_features(self, img_s, img_q):
        if self.backbone_type == "swin":
            ...
            feature_s = [
                self.backbone.get_intermediate_layers(img, n=1)[0][:, 1:]
                .reshape(batch_size, img_height // 8, img_width // 8, -1)
                .permute(0, 3, 1, 2)
                for img in img_s
            ]
            ...
            batch_size, _, img_height, img_width = img_q.shape
            feature_q = self.backbone.get_intermediate_layers(img_q, n=1, reshape=True)[
                0
            ]  # [bs, c, h, w]
            feature_s = [
                self.backbone.get_intermediate_layers(img, n=1, reshape=True)[0]
                for img in img_s
            ]
        else:
    ...
    def show_result(
                ...
                if skeleton is not None and pose_limb_color is not None:
                    ...
                    for sk_id, sk in enumerate(skeleton):
                        ...
                        if (
                            ...
                            transparency = max(
                                0,
                                min(1, 0.5 * (kpts[sk[0] - 1, 2] + kpts[sk[1] - 1, 2])),
                            )
                            cv2.addWeighted(
                                img_copy,
                                transparency,
        ...
        if show:
            ...
            enlarge = cv2.resize(
                img, (0, 0), fx=factor, fy=factor, interpolation=cv2.INTER_CUBIC
            )

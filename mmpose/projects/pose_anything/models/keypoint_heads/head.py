...
class PoseHead(nn.Module):
    ...
    def forward(self, x, feature_s, target_s, mask_s, skeleton):
        ...
        support_order_embedding = x.new_zeros(
            (bs, self.embed_dims, 1, target_s[0].shape[1])
        ).to(torch.bool)
        ...
        for i, (feature, target) in enumerate(zip(feature_s, target_s)):
            ...
            support_keypoints = target.flatten(2) @ resized_feature.flatten(2).permute(
                0, 2, 1
            )
        ...
        masks_query = (~mask_s.to(torch.bool)).squeeze(
            -1
        )  # True indicating this query matched no actual joints.
    ...
    def get_loss(
        ...
        if self.with_heatmap_loss:
            losses["heatmap_loss"] = (
                self.heatmap_loss(
                    similarity_map, target_heatmap, target_weight, normalizer
                )
        ...
        proposal_l1_loss = proposal_l1_loss.sum(
            dim=-1, keepdim=False
        ) * target_weight.squeeze(dim=-1)
        proposal_l1_loss = (
            proposal_l1_loss.sum(dim=-1, keepdim=False) / normalizer
        )  # [bs, ]
        ...
        for idx in range(num_dec_layer):
            ...
            l1_loss = F.l1_loss(
                layer_output, layer_target, reduction="none"
            )  # [bs, query, 2]
            l1_loss = l1_loss.sum(dim=-1, keepdim=False) * target_weight.squeeze(
                dim=-1
            )  # [bs, query]
    ...
    def heatmap_loss(self, similarity_map, target_heatmap, target_weight, normalizer):
        ...
        l2_loss = F.mse_loss(
            similarity_map, target_heatmap, reduction="none"
        )  # bs, nq, h, w
    ...
    def decode(self, img_metas, output, img_size, **kwargs):
        ...
        W, H = img_size
        output = (
            output * np.array([W, H])[None, None, :]
        )  # [bs, query, 2], coordinates with recovered shapes.

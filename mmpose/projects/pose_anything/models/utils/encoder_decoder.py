...
class ProposalGenerator(nn.Module):
    ...
    def forward(self, query_feat, support_feat, spatial_shape):
        ...
        similarity_coord_grid = similarity_softmax[:, :, :, None] * coord_grid.flatten(
            2, 3
        )
        ...
        max_pos = torch.argmax(
            similarity.reshape(bs, nq, -1), dim=-1, keepdim=True
        )  # (bs, nq, 1)
...
class GraphTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        tgt_key_padding_mask_remove_all_true[
            tgt_key_padding_mask.logical_not().sum(dim=-1) == 0, 0
        ] = False
...
class GraphTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        q = k = self.with_pos_embed(
            refined_support_feat, query_pos + pos[refined_query_feat.shape[0] :]
        )
        tgt2 = self.self_attn(
            q,
            k,
        ...
        if self.graph_decoder is not None:
            ...
            if self.graph_decoder == "pre":
                tgt2 = self.ffn2(
                    self.dropout(self.activation(self.ffn1(refined_support_feat, adj)))
                )
            elif self.graph_decoder == "post":
                tgt2 = self.ffn2(
                    self.dropout(self.activation(self.ffn1(refined_support_feat))), adj
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        mask_cat = torch.cat(
            (src_key_padding_mask, query_key_padding_mask), dim=1
        )  # [bs, hw+nq]
        output = src_cat

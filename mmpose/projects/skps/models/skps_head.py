...
class SKPSHead(BaseHead):
    ...
    def loss(
        ...
        # calculate losses
        losses = dict()
        heatmap_loss = self.loss_module["heatmap"](
            pred_heatmaps, gt_heatmaps, keypoint_weights
        )

        n, c, h, w = pred_offside.size()
        offside_loss_x = self.loss_module["offside"](
            pred_offside[:, : c // 2], gt_offside[:, : c // 2], gt_heatmaps
        )

        offside_loss_y = self.loss_module["offside"](
            pred_offside[:, c // 2 :], gt_offside[:, c // 2 :], gt_heatmaps
        )

        offside_loss = (offside_loss_x + offside_loss_y) / 2.0
    ...
    def predict(
        ...
        if flip_test:
            ...
            for b in range(batch_size):
                ...
                _keypoints = (_keypoints + _keypoints_flip) / 2.0
                # pack outputs
            ...
            preds = self.decode(batch_heatmaps, batch_displacements, test_cfg, metainfo)
    ...
    def decode(
        ...
        for b in range(batch_size):
            keypoints, keypoint_scores = self.decoder.decode(heatmaps[b], offside[b])

            # pack outputs
            preds.append(
                InstanceData(keypoints=keypoints, keypoint_scores=keypoint_scores)
            )

...
class YOLOXPoseHead(YOLOXHead):
    ...
    def loss_by_feat(
        ...
        flatten_kpts = self.decode_pose(
            grid_priors[..., :2], kpt_preds, grid_priors[..., 2]
        )
        ...
        if len(kpt_targets) > 0:
            ...
            loss_vis = (
                self.loss_cls(
                    vis_preds.view(-1, self.num_keypoints)[pos_masks], vis_targets
                )
                / vis_targets.sum()
            )
        else:
    ...
    def predict_by_feat(
        ...
        with OutputSaveFunctionWrapper(
            filter_scores_and_topk, super().predict_by_feat.__globals__
        ) as outputs_1:
            with OutputSaveFunctionWrapper(
                batched_nms, super()._bbox_post_process.__globals__
            ) as outputs_2:
        ...
        kpt_preds = torch.cat(
            [
                kpt_pred.permute(0, 2, 3, 1).reshape(
                    num_imgs, -1, self.num_keypoints * 2
                )
                for kpt_pred in kpt_preds
            ],
            dim=1,
    ...
    def predict(self, x: Tuple[Tensor], batch_data_samples, rescale: bool = False):
        predictions = [
            ...
            for pred_instances in super().predict(x, batch_data_samples, rescale)
        ]
        return predictions
    ...
    def decode_pose(
        ...
        strides: Union[torch.Tensor, int],
    ) -> torch.Tensor:
    ...
    def gt_instances_preprocess(
        batch_gt_instances: List[InstanceData], *args, **kwargs
    ) -> List[InstanceData]:

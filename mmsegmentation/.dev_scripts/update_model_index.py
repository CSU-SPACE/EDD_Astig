...
def get_model_info(
    md_file: str, config_dir: str, collection_name_list: List[str]
) -> Tuple[dict, str]:
        ...
        while i < len(lines):
            ...
            if (
                ...
                while j < len(lines) and lines[j][0] == "|":
                    ...
                    # get config name
                    try:
                        config_url = re.findall(
                            r"[a-zA-Z]+://[^\s]*py", values[config_idx]
                        )[0]
                    ...
                    # get model name
                    try:
                        weight_url = re.findall(
                            r"[a-zA-Z]+://[^\s]*pth", values[download_idx]
                        )[0]
                        log_url = re.findall(
                            r"[a-zA-Z]+://[^\s]*.json", values[download_idx + 1]
                        )[0]
                    except IndexError:
                    ...
                    mem = (
                        ...
                        if values[mem_idx] != "-" and values[mem_idx] != ""
                        else -1
                    )
                    ...
                    backone: str = re.findall(
                        r"[^\s]*", values[keys.index("Backbone")].strip()
                    )[0]
                    archs = [backone] + method
                    collection_name = method[0]
                    config_path = osp.join(
                        "configs", config_dir.split("/")[-1], config_name
                    )
                    model = {
                        "Name": model_name,
...
def update_model_index(config_dir_list: List[str]) -> bool:
    """update model index."""
    yml_files = [
        osp.join("configs", dir_name.split("/")[-1], "metafile.yaml")
...
if __name__ == "__main__":
    ...
    for md_file, config_dir in zip(md_file_list, config_dir_list):
        results, collection_name = get_model_info(
            md_file, config_dir, collection_name_list
        )
        if results is None:

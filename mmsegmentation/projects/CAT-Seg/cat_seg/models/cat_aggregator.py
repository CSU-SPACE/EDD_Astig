...
class AGWindowMSA(BaseModule):
    ...
    def __init__(
        ...
        self.qk = nn.Linear(embed_dims + appearance_dims, embed_dims * 2, bias=qkv_bias)
    ...
    def forward(self, x, mask=None):
        ...
        qk = (
            self.qk(x)
            .reshape(B, N, 2, self.num_heads, self.embed_dims // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )  # 2 B NUM_HEADS N embed_dims//NUM_HEADS
        v = (
            self.v(x[:, :, : self.embed_dims])
            .reshape(B, N, self.num_heads, self.embed_dims // self.num_heads)
            .permute(0, 2, 1, 3)
        )  # B NUM_HEADS N embed_dims//NUM_HEADS
...
class AGShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape):
        ...
        attn_windows = attn_windows.view(
            -1, self.window_size, self.window_size, self.w_msa.embed_dims
        )
...
class SpatialAggregateLayer(BaseModule):
    ...
    def __init__(
        ...
        self.guidance_norm = (
            nn.LayerNorm(appearance_dims) if appearance_dims > 0 else None
        )
    ...
    def forward(self, x, appearance_guidance):
        ...
        if appearance_guidance is not None:
            appearance_guidance = appearance_guidance.repeat(T, 1, 1, 1).permute(
                0, 2, 3, 1
            )  # BT, HW, C
...
class ClassAggregateLayer(BaseModule):
    ...
    def __init__(
        ...
        self.attention = AttentionLayer(
            hidden_dims, guidance_dims, nheads=num_heads, attention_type=attention_type
        )
        self.MLP = FFN(
            embed_dims=hidden_dims, feedforward_channels=hidden_dims * 4, num_fcs=2
        )
    ...
    def forward(self, x, guidance):
        ...
        *_, H_pool, W_pool = x_pool.size()
        ...
        x_pool = (
            x_pool.reshape(B, H_pool * W_pool, T, C)
            .permute(0, 2, 3, 1)
            .reshape(B, T, C, H_pool, W_pool)
            .flatten(0, 1)
        )  # BT C H_p W_p
...
class CATSegAggregator(BaseModule):
    ...
    def feature_map(self, img_feats, text_feats):
        ...
        img_feats = img_feats.unsqueeze(2).repeat(1, 1, text_feats.shape[1], 1, 1)
        ...
        text_feats = (
            ...
            .unsqueeze(-1)
            .repeat(1, 1, 1, img_feats.shape[-2], img_feats.shape[-1])
            .transpose(1, 2)
        )
    ...
    def corr_embed(self, x):
        ...
        corr_embed = corr_embed.reshape(
            B, -1, self.embed_dims, x.shape[-2], x.shape[-1]
        ).transpose(1, 2)
        return corr_embed
    ...
    def forward(self, inputs):
        ...
        B = img_feats.size(0)
        appearance_guidance = inputs["appearance_feat"][
            ::-1
        ]  # order (out_indices) 2, 1, 0
        text_feats = (
            inputs["clip_text_feat"] if self.training else inputs["clip_text_feat_test"]
        )
        ...
        return dict(corr_embed=corr_embed, appearance_feats=appearance_guidance[1:])

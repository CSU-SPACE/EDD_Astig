...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, guidance_channels):
        ...
        self.up = nn.ConvTranspose2d(
            in_channels, in_channels - guidance_channels, kernel_size=2, stride=2
        )
        ...
        self.decoder_guidance_projection = (
            nn.ModuleList(
                [
                    nn.Sequential(
                        nn.Conv2d(
                            dec_dims, dec_dims_proj, kernel_size=3, stride=1, padding=1
                        ),
                        nn.ReLU(),
                    )
                    for dec_dims, dec_dims_proj in zip(
                        decoder_guidance_dims, decoder_guidance_proj_dims
                    )
                ]
            )
            if decoder_guidance_dims[0] > 0
            else None
        )

        self.decoder1 = UpBlock(
            embed_dims, decoder_dims[0], decoder_guidance_proj_dims[0]
        )
        self.decoder2 = UpBlock(
            decoder_dims[0], decoder_dims[1], decoder_guidance_proj_dims[1]
        )
    ...
    def forward(self, inputs):
        ...
        if self.decoder_guidance_projection is not None:
            projected_decoder_guidance = [
                proj(g)
                for proj, g in zip(
                    self.decoder_guidance_projection, inputs["appearance_feats"]
                )
            ]

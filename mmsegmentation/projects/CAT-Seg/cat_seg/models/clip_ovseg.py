...
class CLIPOVCATSeg(BaseModule):
    ...
    def __init__(
        ...
        clip_pixel_mean: List[float] = [122.7709383, 116.7460125, 104.09373615],
        ...
        self.register_buffer(
            "clip_pixel_mean", torch.Tensor(clip_pixel_mean).view(1, -1, 1, 1), False
        )
        self.register_buffer(
            "clip_pixel_std", torch.Tensor(clip_pixel_std).view(1, -1, 1, 1), False
        )
        ...
        if clip_pretrained == "ViT-G" or clip_pretrained == "ViT-H":
            ...
            try:
                ...
                print(f"Will load {pretrain} weights from {custom_clip_weights}.")
                assert (
                    custom_clip_weights is not None
                ), "Please specify custom weights directory."  # noqa
                assert os.path.exists(
                    os.path.join(custom_clip_weights, "open_clip_pytorch_model.bin")
        ...
        text_features = (
            self.class_embeddings(
                self.class_texts, prompt_templates, clip_model, device
            )
            .permute(1, 0, 2)
            .float()
        )
        text_features_test = (
            self.class_embeddings(
                self.test_class_texts, prompt_templates, clip_model, device
            )
            .permute(1, 0, 2)
            .float()
        )
        ...
        finetune_backbone = backbone_multiplier > 0.0
    ...
    def forward(self, inputs):
        ...
        clip_features = self.clip_model.encode_image(
            clip_inputs, dense=True
        )  # B, 577(24x24+1), C
        B = clip_features.size(0)
        outputs["clip_img_feat"] = (
            clip_features[:, 1:, :]
            .permute(0, 2, 1)
            .reshape(B, -1, *self.clip_img_feat_size)
        )

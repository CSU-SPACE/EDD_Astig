...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.prompt_tokens = (
            nn.Parameter(torch.zeros(prompt_depth, prompt_length, width))
            if prompt_length > 0
            else None
        )
        ...
        for i, resblock in enumerate(self.resblocks):
            if self.prompt_length > 0 and i < self.prompt_depth:
                ...
                x = torch.cat(
                    (
                        x[0:1, :, :],
                        self.prompt_tokens[i].repeat(x.shape[1], 1, 1).permute(1, 0, 2),
                        x[length:, :, :],
                    )
                )
...
class VisualTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            width, layers, heads, prompt_depth=prompt_depth, prompt_length=prompt_length
        )
    ...
    def forward(self, x: torch.Tensor, dense=False):
        ...
        if dense and (x.shape[1] != self.positional_embedding.shape[0]):
            x = x + self.resized_pos_embed(self.input_resolution, x.shape[1]).to(
                x.dtype
            )
        else:

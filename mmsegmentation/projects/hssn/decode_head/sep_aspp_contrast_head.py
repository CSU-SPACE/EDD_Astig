...
class ProjectionHead(nn.Module):
    ...
    def __init__(
        self, dim_in: int, norm_cfg: dict, proj_dim: int = 256, proj: str = "convmlp"
    ):
        super().__init__()
...
class DepthwiseSeparableASPPContrastHead(DepthwiseSeparableASPPHead):
    ...
    def predict_by_feat(
        self, seg_logits: Tuple[Tensor], batch_img_metas: List[dict]
    ) -> Tensor:
        ...
        if seg_logit.size(1) == 26:  # For cityscapes dataset，19 + 7
            ...
            seg_logit[:, 10:12] = (
                seg_logit[:, 10:12] + seg_logit[:, 21] + seg_logit[:, 24]
            )
            seg_logit[:, 13:16] = (
                seg_logit[:, 13:16] + seg_logit[:, 21] + seg_logit[:, 24]
            )
            seg_logit[:, 8:10] = (
                seg_logit[:, 8:10] + seg_logit[:, 22] + seg_logit[:, 24]
            )
            seg_logit[:, 12:13] = (
                seg_logit[:, 12:13] + seg_logit[:, 22] + seg_logit[:, 24]
            )
            seg_logit[:, 16:20] = (
                seg_logit[:, 16:20] + seg_logit[:, 22] + seg_logit[:, 24]
            )

...
def reformulate_file(image_list, mask_list):
    file_list = []
    for idx, (imgp, maskp) in enumerate(zip(sorted(image_list), sorted(mask_list))):
    ...
    for ith, part in enumerate(file_lists):
        ...
        for sample in part:
            ...
            img_save_path = os.path.join(
                root_path, "images", part_dir, targetname + save_img_suffix
            )
            ...
            if mask_path is not None:
                mask_save_path = os.path.join(
                    root_path, "masks", part_dir, targetname + save_seg_map_suffix
                )
...
if __name__ == "__main__":
    ...
    train_masks = glob.glob(
        "data/ISIC-2017_Training_Part1_GroundTruth/*" + seg_map_suffix
    )

    val_imgs = glob.glob("data/ISIC-2017_Validation_Data/*" + img_suffix)
    val_masks = glob.glob(
        "data/ISIC-2017_Validation_Part1_GroundTruth/*" + seg_map_suffix
    )

    test_imgs = glob.glob("data/ISIC-2017_Test_v2_Data/*" + img_suffix)
    test_masks = glob.glob(
        "data/ISIC-2017_Test_v2_Part1_GroundTruth/*" + seg_map_suffix
    )
    ...
    part_dir_dict = {0: "train/", 1: "val/", 2: "test/"}
    process_dataset([train_pair_list, val_pair_list, test_pair_list], part_dir_dict)

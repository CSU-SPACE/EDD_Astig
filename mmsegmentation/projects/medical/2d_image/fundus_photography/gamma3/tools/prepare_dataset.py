...
def convert_label_pics_into_pngs(
    src_dir, tgt_dir, suffix, convert_dict={0: 2, 128: 1, 255: 0}
):
    if not os.path.exists(tgt_dir):
        os.makedirs(tgt_dir)
...
if __name__ == "__main__":
    ...
    convert_pics_into_pngs(
        os.path.join(
            root_path, "task3_disc_cup_segmentation/training/fundus color images/"
        ),
        tgt_img_train_dir,
        suffix=img_suffix,
    )
    ...
    convert_pics_into_pngs(
        os.path.join(
            root_path, "task3_disc_cup_segmentation/testing/fundus color images/"
        ),
        tgt_img_test_dir,
        suffix=img_suffix,
    )
    ...
    convert_label_pics_into_pngs(
        os.path.join(root_path, "task3_disc_cup_segmentation/training/Disc_Cup_Mask/"),
        tgt_mask_train_dir,
        suffix=seg_map_suffix,
        convert_dict={0: 2, 128: 1, 255: 0},
    )

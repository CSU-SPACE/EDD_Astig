...
x_train = glob.glob(
    os.path.join("data/Vessels-Datasets/*/Train/Original/Images/*" + img_suffix)
)
x_test = glob.glob(
    os.path.join("data/Vessels-Datasets/*/Test/Original/Images/*" + img_suffix)
)
...
for ith, part in enumerate([x_train, x_test]):
    ...
    for img in part:
        ...
        for seg_map_suffix in seg_map_suffix_list:
            if os.path.exists(
                "/".join(img.split("/")[:-1]).replace("Images", "Labels")
            ):

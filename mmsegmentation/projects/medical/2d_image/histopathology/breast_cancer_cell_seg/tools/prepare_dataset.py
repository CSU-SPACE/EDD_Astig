...
x_train = glob.glob(
    os.path.join("data/Breast Cancer Cell Segmentation_datasets/Images/*" + img_suffix)
)
...
for ith, part in enumerate([x_train]):
    ...
    for img in part:
        ...
        mask_path = (
            root_path
            + "Breast Cancer Cell Segmentation_datasets/Masks/"
            + "_".join(basename.split("_")[:-1])  # noqa
            + seg_map_suffix
        )

...
def save_anno(img_list, file_path, remove_suffix=True):
    if remove_suffix:
        img_list = ["/".join(img_path.split("/")[-2:]) for img_path in img_list]
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(data_root, "masks/val")) and not os.path.exists(
        os.path.join(data_root, "masks/test")
    ):
        all_imgs = sorted(glob.glob(data_root + "/images/train/*.png"))
        x_train, x_val = train_test_split(all_imgs, test_size=0.2, random_state=0)

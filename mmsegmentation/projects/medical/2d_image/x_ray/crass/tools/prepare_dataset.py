...
def filter_suffix_recursive(src_dir, suffix):
    ...
    file_paths = glob(os.path.join(src_dir, "**", "*" + suffix), recursive=True)
...
def convert_meds_into_pngs(
    src_dir, tgt_dir, suffix=".dcm", norm_min=0, norm_max=255, convert="RGB"
):
    ...
    for i, (src_name, src_path) in enumerate(zip(src_names, src_paths)):
        ...
        if norm_min is not None and norm_max is not None:
            img = cv2.normalize(
                img, None, norm_min, norm_max, cv2.NORM_MINMAX, cv2.CV_8U
            )

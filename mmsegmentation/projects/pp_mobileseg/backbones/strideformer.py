...
class SeaAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        krow = self.pos_emb_rowk(k.mean(-1)).reshape(
            [B, self.num_heads, -1, H]
        )  # [B, nhead, dim, H]
        vrow = (
            v.mean(-1).reshape([B, self.num_heads, -1, H]).permute([0, 1, 3, 2])
        ...
        vcolumn = (
            torch.mean(v, -2).reshape([B, self.num_heads, -1, W]).permute([0, 1, 3, 2])
        )
...
class InjectionMultiSumallmultiallsum(nn.Module):
    ...
    def forward(self, inputs):  # x_x8, x_x16, x_x32, x_x64
        ...
        high_feat = F.interpolate(
            self.embedding_list[2](inputs[2]), size=low_feat2.shape[2:], mode="bilinear"
        )

        res = (
            low_feat1_act * low_feat2_act * high_feat_act * (low_feat1 + low_feat2)
            + high_feat
        )
...
class InjectionMultiSumallmultiallsumSimpx8(nn.Module):
    ...
    def forward(self, inputs):
        ...
        high_feat = F.interpolate(
            self.embedding_list[1](inputs[2]), size=low_feat2.shape[2:], mode="bilinear"
        )

...
def test_dice_loss(naive_dice):
    ...
    for use_sigmoid in [True, False]:
        loss_class(use_sigmoid=use_sigmoid, activate=True, naive_dice=naive_dice)(
            pred, target
        )
        assert isinstance(loss, torch.Tensor)
    ...
    weight = torch.rand(1)
    loss = loss_class(naive_dice=naive_dice)(pred, target, weight)
    loss_onehot = loss_class(naive_dice=naive_dice)(pred, target_onehot, weight)
    ...
    weight = torch.rand(1)
    loss = loss_class(naive_dice=False, use_sigmoid=True, eps=0)(pred, target, weight)
    assert loss.item() == 0
    ...
    with pytest.raises(AssertionError):
        ...
        weight = torch.rand(1)
        loss = loss_class(
            naive_dice=naive_dice, use_sigmoid=False, ignore_index=0, eps=0
        )(pred, target, weight)
    ...
    weight = torch.rand(1)
    loss = loss_class(naive_dice=False, use_sigmoid=True, ignore_index=1, eps=0)(
        pred, target, weight
    )
    assert loss.item() == 0

...
def main():
    ...
    parser.add_argument(
        "--out-file", default="prediction.png", help="Path to output prediction file"
    )
    parser.add_argument(
    ...
    input_tensor = preprocess_image(
        rgb_img, mean=[x / 255 for x in image_mean], std=[x / 255 for x in image_std]
    )

    # Grad CAM(Class Activation Maps)
    # Can also be LayerCAM, XGradCAM, GradCAMPlusPlus, EigenCAM, EigenGradCAM
    targets = [SemanticSegmentationTarget(category, mask_float, (height, width))]
    with GradCAM(

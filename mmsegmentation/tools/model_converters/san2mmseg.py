    ...
    for k, v in ckpt.items():
        ...
        if key_list[0] == "clip_visual_extractor":
            ...
            if key_list[1] == "class_embedding":
                ...
                new_name = ".".join([new_transform_name, key_list[1], key_list[2]])
                ...
                if paras[0] == "ln_1":
                    ...
                    if paras[1] == "c_fc":
                        new_para_name = ".".join(["ffn.layers.0.0"] + key_list[-1:])
                    else:
                        new_para_name = ".".join(["ffn.layers.1"] + key_list[-1:])
                new_name = ".".join(
                    [new_transform_name, new_layer_name, layer_index, new_para_name]
                )
            ...
            if key_list[1] == "vit_model":
                if key_list[2] == "blocks":
                    ...
                    if paras[0] == "norm1":
                        ...
                        new_para_name = new_para_name.replace("fc1", "layers.0.0")
                        new_para_name = new_para_name.replace("fc2", "layers.1")
                    else:
                        print(f"Wrong for {k}")
                    new_name = ".".join(
                        [
                    ...
                    new_name = ".".join([decode_head_name, module_name, "pos_embed"])
                    ...
                    print(f"Wrong for {k}")
            elif key_list[1] == "query_embed" or key_list[1] == "query_pos_embed":
                new_name = ".".join([decode_head_name, module_name, key_list[1]])
            ...
            if key_list[1] == "proj":
                new_name = ".".join([decode_head_name, module_name, "proj.weight"])
